""" Windowing function for supporting kpick.
    ...
    for i3 in range(num_windows):
        for i2 in range(num_test):
            for j1 in range(num_train): 
                d = np.linalg.norm(test_set[<unsigned int>i3,<unsigned int>i2,]-train_set[<unsigned int>j1,])
                D[<unsigned int>i3,<unsigned int>i2,<unsigned int>j1] = d**2
